...
ET_Save_File_Tag_From_UI (File_Tag *FileTag)
{
    ...
    if ( g_utf8_strlen(buffer, -1) > 0  )
    {
        FileTag->track = et_format_track_number(atoi(buffer));
        g_free(buffer);
    } else
    {
        FileTag->track = NULL;
    ...
    if ( g_utf8_strlen(buffer, -1) > 0  )
    {
        FileTag->track_total = et_format_track_number(atoi(buffer));
        g_free(buffer);
    } else
    {
        FileTag->track_total = NULL;
...
ET_Save_File_Tag_Internal (ET_File *ETFile, File_Tag *FileTag)
{
    ...
    {
        FileTag->track_total = et_format_track_number(atoi(FileTagCur->track_total));
